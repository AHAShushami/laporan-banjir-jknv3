 ...
 */
import { getSession, showToast, formatDateTime, formatDateInput, getToday, getTimePeriod, showConfirm } from '../utils.js?v=23';
import { FORM_CONFIGS } from '../config/forms.js?v=23';
...
import { generateExcelReport } from '../excel-generator.js?v=23';
...
window.app.downloadExcelReport = async (selectedDate) => {
    ...
    try {
        const allSubs = [];
        const { FORM_ORDER } = await import('../config/forms.js?v=23');
        for (const formId of FORM_ORDER) {
            const subs = await store.getSubmissions(formId);
            allSubs.push(...subs);
        }

        const { DISTRICTS } = await import('../config/districts.js?v=23');
        const ppsList = [];
